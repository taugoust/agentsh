...
`--runtime` defaults to `13h` for old wrappers, must be a positive whole-second Go duration, and cannot exceed `192h`. Bootstrap schema 2 added `started_at`, `expires_at`, `runtime_seconds`, and `bootstrap_schema_version`; schema 3 adds the helper-selected `composition_scratch_root` while retaining helper protocol version 1. The root bootstrap creates that lease child as a root-owned `1733` directory, and the lease's systemd `RuntimeDirectory` removes it at teardown. `RuntimeMaxSec` and `expires_at` derive from the same start instant and duration. Capability JSON advertises `bootstrap_runtime`, `composition_runtime`, `bootstrap_default_runtime_seconds`, `bootstrap_max_runtime_seconds`, and `instance_lifecycle`.

With `sandbox.composition.bubblewrap.scratch_root: auto`, the supervisor accepts only the active authenticated schema-3 lease path `/run/agentsh/nethelper/<uid>/<lease>/composition`. It validates the fixed topology, lease identity/expiry, UID/GID, root ownership, mode, and symlink safety at startup and after helper rebinding. The stable per-UID helper root remains an internal deny; only the pinned wrapper setup path can use the exact composition child. Strict payload mount masking hides helper credentials, sockets, metadata, and other lease controls individually without hiding the sibling composition child before trusted setup. Project policy must not grant or mention this control topology.

Automatic runtime startup persists `composition_runtime_provisioned` and `composition_runtime_ready`; malformed runtime validation persists `composition_runtime_failed` and aborts startup. Each composed command removes synthetic construction names before Landlock enforcement, verifies their absence in the broker, and persists `composition_runtime_cleanup` with bounded command/pool evidence. Failure to persist mandatory runtime evidence fails closed.
