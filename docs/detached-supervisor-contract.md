...
- Each authenticated child is one serialized execution lane. Different child lanes may overlap up to `sessions.subagents.max_exec_concurrency`. The fail-closed default is `1`. Parent/root requests and unsupported transports remain exclusive. Strict Linux eBPF execution gives every overlapping command an immutable-attribution proxy listener and permits only that exact endpoint in its command cgroup. Persistent FUSE, ptrace/ESF, netns, and non-strict transparent-interception paths remain serialized.
