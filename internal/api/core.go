...
func (a *App) execInSessionCoreWithOptions(ctx context.Context, id string, req types.ExecRequest, opts internalExecOptions) (*types.ExecResponse, int, error) {
	...
	unlock, lockErr := s.LockExecContext(queueCtx)
	if lockErr != nil {
		return nil, http.StatusRequestTimeout, fmt.Errorf("execution queue timeout: %w", lockErr)
	}
	defer unlock()
	...
	exitCode, stdoutB, stderrB, stdoutTotal, stderrTotal, stdoutTrunc, stderrTrunc, resources, execErr := runCommandWithResourcesResolvedTimeout(ctx, s, cmdID, wrappedReq, a.cfg, cmdDecision.EnvPolicy, timeoutResolution.Duration, a.cgroupHook(id, cmdID, limits), extraCfg, a.ptraceTracer, id)
