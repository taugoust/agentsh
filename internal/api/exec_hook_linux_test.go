...
func TestRunCommandWithResourcesStreamingHookErrorFailsClosed(t *testing.T) {
	...
	exitCode, _, _, _, _, _, _, _, err := runCommandWithResourcesStreamingEmit(context.Background(), s, "cmd-hook-fail-stream", req, &config.Config{}, policy.ResolvedEnvPolicy{}, 0, nil, hook, nil, nil, "", nil)
	assertHookFailureResult(t, exitCode, err, marker)
}
